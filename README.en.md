...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 077 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Ordinary collage often just cuts a photograph into pieces or piles on sticker-like decoration. It may look busy, but lacks meaningful depth, reading order, and subject identity.

**Panel 077** reduces subject, action, contour, and scene relationship into paper-sculpture collage. Clear cut-paper silhouettes, front–back planes, subtle paper texture, cut edges, and soft shadows create depth. Background is deliberately reduced, while macaron colour is recomposed from the source's most vital and emotionally warm clues. The result reads like a refined travel magazine or independent art journal, not a pile of cartoon stickers.

### Best for

- turning travel photographs into authored paper editorial rather than reduced photo thumbnails;
- gaining tactile planar depth through cut edges, layered paper, and soft shadow without heavy plastic 3D;
- using gentle bright colour derived from the source's emotional warmth rather than a fixed candy palette;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 077 on this image and recommend the most suitable composition and size first.
...
**Signature:** minimalist paper sculpture · clear cut-paper contour · front–back layers · soft shadow · humanist macaron colour · travel-magazine typography
...
|---|---|
| **What you get** | A refined collage poster with tactile paper depth, clear narrative, and travel-editorial character |
| **Recognisable signature** | Summarised forms, clean cut edges, layered paper planes, soft shadows, and low-weight magazine typography |
...
```text
/xxd-panel-077 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-077 photo.jpg --prefs last
/xxd-panel-077 photo.jpg --prefs edit
/xxd-panel-077 photo.jpg --prefs new
```
...
/xxd-panel-077 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-077 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-077 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-077 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-077 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-077 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-077 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-077 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-077 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 077 to a whole directory; shared settings resolve once
/xxd-panel-077 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-077 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-077.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-077" ~/.codex/skills/xxd-panel-077
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-077 --skill xxd-panel-077
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-077`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-077-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-077-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 077; the current project is bold.
