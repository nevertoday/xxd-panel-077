...
# 🦁 XXD Panel 077｜여행 잡지 일러스트

### 사진을 부드러운 그림자와 인간적 색을 지닌 종이 조각 여행 잡지로 바꿉니다
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 077 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 077 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 077 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 077 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 077 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 077 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 077 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 077 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 077이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 077**는 피사체, 행동, 윤곽과 장면 관계를 종이 조각 콜라주로 요약합니다. 명확한 종이 오리기 윤곽, 앞뒤 종이 면, 은은한 종이 결, 잘린 가장자리와 부드러운 그림자가 깊이를 만듭니다. 배경을 덜고 원본의 생명력과 감정 온도에서 마카롱 색을 재조율해 여행 잡지나 독립 예술 저널처럼 완성합니다.

### 이런 경우에 적합합니다

- 여행 사진을 축소된 사진이 아니라 피사체와 장면 관계를 다시 조직한 종이 공예 출판물로 만들고 싶을 때
- 평면이지만 실제 층위가 필요하며 종이 절단면, 앞뒤 겹침과 부드러운 그림자로 촉감을 만들되 무거운 플라스틱 3D는 피하고 싶을 때
- 고정된 사탕색이 아니라 원본의 생명력과 감정 온도에서 나온 부드럽고 밝은 마카롱 색을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 077로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 종이 촉감, 명확한 서사, 여행 출판물의 기운을 지닌 고급 콜라주 포스터 |
| **대표 특징** | 요약된 형태, 잘린 종이 가장자리, 앞뒤 종이 층, 부드러운 그림자, 낮은 무게의 잡지 글자 |
| **입력 존중 방식** | 피사체, 행동, 윤곽, 장면 관계를 유지하고 감정과 서사를 지지하지 않는 배경만 덜어냄 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-077`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-077-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-077-prompt.zh-CN.md)
...
**특징:** 미니멀 종이 조각 · 명확한 종이 오리기 윤곽 · 앞뒤 층 · 부드러운 그림자 · 인간적 마카롱 · 여행 잡지 타이포그래피
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 077의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-077 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-077 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-077 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-077 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 077 적용, 공통 설정은 한 번만 확정
/xxd-panel-077 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–077을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
