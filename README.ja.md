...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 077 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
主体、動作、輪郭、場面関係を抽出 → 切り紙形へ概括 → 前後紙層と局部裁切を整理 → 紙目、切断面、柔らかな影で手工奥行き → 元写真の感情色をマカロンへ再調整 → 静かな余白 → 低重量の旅行誌組版
```

## 完成作の特徴

- 概括形と明瞭な切り紙輪郭で同一性を保ち、写実やカートゥーンへ寄せない。
- 前後紙層、切断面、軽い紙目、柔らかな影で抑制された触覚的奥行きをつくる。
- 感情や場面関係を支えない背景と副次情報は削る。
- マカロン色は軽く温かく新鮮で、汚れ、古び、蛍光、安価な甘さを避ける。
- 大見出し一つと少量のタイプライター文字を低重量で紙面に統合する。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-077-prompt.en.md)
...
77 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 077 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–077） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
