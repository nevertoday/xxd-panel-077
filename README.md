...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 077 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

普通拼贴常常只是把照片切成几块，或堆叠廉价贴纸元素；看似丰富，却没有清晰的层次、阅读路径和主体身份。

**Panel 077** 把主体、人物动作、轮廓结构和场景关系概括成纸雕拼贴：清晰剪纸轮廓、前后层叠块面、轻微纸纹、切边感与柔和投影共同建立空间。背景主动删减，马卡龙颜色从照片最有生命力和情感温度的部分重新调制，最终像一本高级旅行杂志或独立艺术刊物，而不是卡通贴纸拼盘。

### 适合这些情况

- **想把旅行照片做成纸艺刊物：** 不是缩小照片，而是重新组织主体与场景关系。
- **需要平面但有真实层次：** 纸张切边、前后叠层和柔和投影建立触感，不做厚重塑料 3D。
- **喜欢温柔明亮又不廉价：** 马卡龙色从原图生命力与情感温度中生成，不套固定糖果色。
...
> 用 XXD Panel 077 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 极简纸雕拼贴 · 清晰剪纸轮廓 · 前后层叠 · 柔和投影 · 人文马卡龙 · 旅行杂志排版

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 077 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅具有纸艺触感、清晰叙事与旅行刊物气质的高级拼贴海报 |
| **一眼可辨的特点** | 概括形体、清晰剪纸边缘、前后纸层、柔和投影与低权重杂志文字 |
| **它如何尊重你的输入** | 保留主体身份、动作、轮廓和场景关系；只删去不支持情绪与叙事的背景细节 |
...
```text
提取主体、动作、轮廓与场景关系 → 概括为清晰剪纸形体 → 组织前后层叠块面与局部裁切 → 用纸纹、切边和柔和投影建立手工层次 → 从原图情感色彩重调明亮马卡龙 → 以安静留白形成阅读路径 → 加入低权重旅行杂志排版
```

## 成品中最容易识别的风格特点

- 主体以概括形体和清晰剪纸轮廓保留身份，不做写实描摹，也不退化成卡通符号。
- 前后纸层、切边、轻微纸纹与柔和投影构成真实但克制的纸雕深度。
- 背景和次要细节主动删减，只留下能支撑情绪与场景关系的少量元素。
- 马卡龙配色轻盈、温暖、清新且被阳光照亮，避免灰脏、发旧、荧光和廉价糖果感。
- 一个简洁标题与少量打字机小字保持低权重，与主体、留白和拼贴块面共同构图。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-077-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 077 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-077 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-077 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 077；公共设置只解析一次
/xxd-panel-077 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-077`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-077-prompt.en.md) · [中文运行适配器](references/xxd-panel-077-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

77 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–077 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–077） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
