...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 077이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
```text
피사체·행동·윤곽·장면 관계 추출 → 종이 오리기 형태로 요약 → 앞뒤 종이 층과 부분 자르기 → 종이 결·절단면·부드러운 그림자로 수공 깊이 → 원본 감정색을 마카롱으로 재조율 → 조용한 여백 → 낮은 무게의 여행 잡지 타이포그래피
```

## 완성작의 특징

- 요약된 형태와 명확한 종이 오리기 윤곽으로 정체성을 보존하며 사실 묘사나 카툰으로 흐르지 않습니다.
- 앞뒤 종이 층, 절단면, 은은한 종이 결, 부드러운 그림자가 절제된 촉각 깊이를 만듭니다.
- 감정과 장면 관계를 지지하지 않는 배경과 부차 정보는 제거합니다.
- 마카롱 색은 가볍고 따뜻하고 신선하며 탁함, 낡음, 형광, 값싼 단맛을 피합니다.
- 큰 제목 하나와 소량의 타자기 글자를 낮은 무게로 종이 면과 통합합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-077-prompt.en.md)
...
77개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 077까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–077) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
