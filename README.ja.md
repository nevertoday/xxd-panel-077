...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 077 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
**Panel 077** は主体、動作、輪郭、場面関係を紙彫刻コラージュへ概括します。明瞭な切り紙輪郭、前後の紙面、軽い紙目、切断面、柔らかな影が奥行きをつくります。背景を削り、元写真の生命力と感情温度からマカロン色を再調整し、旅行誌や独立美術誌のように仕上げます。

### 向いている用途

- 旅行写真を、写真の縮小版ではなく被写体と場面関係を再構成した紙細工の出版物にしたい場合
- フラットでありながら本物の奥行きが必要で、紙の切断面、前後の重なり、柔らかな影による触感を、重いプラスチック 3D なしで表したい場合
- 固定のキャンディ色ではなく、元写真の生命力と感情の温度から生まれる、優しく明るいマカロン色を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 077 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 紙の触感、明瞭な物語、旅行刊行物の気配を持つ上質なコラージュポスター |
| **識別しやすい特徴** | 概括形、切り紙の縁、前後紙層、柔らかな影、低重量の雑誌文字 |
| **入力の尊重** | 主体、動作、輪郭、場面関係を保ち、感情と物語を支えない背景だけを削る |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-077`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-077-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-077-prompt.zh-CN.md)
...
**特徴：** ミニマル紙彫刻 · 明瞭な切り紙輪郭 · 前後レイヤー · 柔らかな影 · 人間的マカロン色 · 旅行誌組版
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 077 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-077 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-077 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-077 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 077 を適用し、共通設定は一度だけ解決
/xxd-panel-077 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜077 を連続して掲載し、現在のプロジェクトを太字で示します。
